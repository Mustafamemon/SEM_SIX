...
def nQueen(board,n,queen_placed,current_col):
    # flag = 0 
    
    # if n == queen_placed:
    #     return True
    
    while n != queen_placed:
        flag = 0 
        for i in range(0,n):
            if board[i][current_col] == '0' and board[i][current_col] != '!':
        ...
        if flag == 0:  # backtrack
            for j in range(0,n):
                if board[j][current_col] == '!':
                    board[j][current_col] = '0'
            current_col -= 1
            if current_col<0:
                return False
    ...
    # return nQueen(board,n,queen_placed,current_col+1)
